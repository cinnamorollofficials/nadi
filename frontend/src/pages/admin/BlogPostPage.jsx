...
const BlogPostPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        const colorClass =
          colors[statusStr] || "bg-surface-variant/30 text-surface-on-variant";
        return (
          <span
            className={`px-2.5 py-1 text-[10px] rounded-full font-bold uppercase tracking-wider ${colorClass}`}
          >
            {String(row.status)}
          </span>
        );
  ...
  const handleSyncCache = async () => {
    ...
    try {
      await syncCache("blogposts");
      toast.success("Blog post cache refreshed successfully");
      setRefreshTrigger((t) => t + 1);
    } catch (error) {
  ...
  const tableActions = [
    ...
    ...(can(PERMS.BLOGPOST_EDIT) ? [{ label: "Edit", onClick: handleOpenModal }] : []),
    ...(can(PERMS.BLOGPOST_DELETE)
      ? [
          {
            label: "Delete",
  ...
  if (!loading && !can(PERMS.BLOGPOST_VIEW)) {
