...
const SECTIONS = [
    { key: 'description', label: 'Deskripsi', icon: '📖', color: 'text-teal-600', bg: 'bg-teal-500/10' },
    { key: 'benefits', label: 'Manfaat', icon: '✨', color: 'text-emerald-600', bg: 'bg-emerald-500/10' },
    { key: 'sources', label: 'Sumber Nutrisi', icon: '🫐', color: 'text-blue-600', bg: 'bg-blue-500/10' },
    { key: 'daily_needs', label: 'Kebutuhan Harian', icon: '📊', color: 'text-amber-600', bg: 'bg-amber-500/10' },
    { key: 'risks_deficiency', label: 'Risiko Kekurangan', icon: '⚠️', color: 'text-red-600', bg: 'bg-red-500/10' },
];
    ...
    if (!data) {
        return (
            <div className="min-h-screen bg-surface flex flex-col items-center justify-center text-center px-6">
                ...
                <h1 className="text-2xl font-bold text-surface-on mb-3">Nutrisi Tidak Ditemukan</h1>
                <p className="text-surface-on-variant mb-8">Informasi yang Anda cari tidak tersedia atau belum dipublikasikan.</p>
                <Link to="/medicpedia/nutrisi" className="inline-flex items-center gap-2 bg-emerald-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-emerald-700 transition">
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-emerald-950 via-teal-900 to-emerald-900 text-white">
                ...
                <div className="relative z-10 max-w-5xl mx-auto px-6 py-16">
                    ...
                    <nav className="flex items-center gap-2 text-sm text-white/60 mb-6">
                        <Link to="/medicpedia/nutrisi" className="hover:text-white transition">Nutrisi A-Z</Link>
                    ...
                    <div className="flex items-start gap-6">
                        ...
                        <div>
                            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-3 py-1 text-xs font-semibold mb-3 uppercase tracking-wider">
                                <span className="w-2 h-2 bg-emerald-400 rounded-full" /> Dipublikasikan
                            </div>
                            <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight leading-tight mb-3">{data.name}</h1>
                            <p className="text-white/70 text-sm">Panduan lengkap dalam {SECTIONS.filter(s => data[s.key]).length} bagian informatif.</p>
                        </div>
                    </div>
                </div>
            </div>
